...
export class UserList implements OnInit {
  ...
  loadUsers() {
    ...
    // Sayfa ilk kez yükleniyorsa listeyi temizle
    ...
    this.userService.getAllUsers(this.currentPage, this.pageSize, search).subscribe({
      next: (response: any) => {
        console.log('User API Response:', response); // Konsoldan kontrol edelim

        // DÜZELTME BURADA:
        // Backend bazen 'content', bazen 'messages' içinde veri döndürüyor.
        // Eğer 'messages' varsa ve doluysa onu al, yoksa 'content'e bak.
        if (response.messages && response.messages.length > 0) {
          this.paginatedUsers = response.messages;
        ...
        this.hasMoreUsers = this.totalPages > 0 && this.currentPage < this.totalPages - 1;
  ...
  loadMoreUsers() {
    ...
    this.userService.getAllUsers(nextPage, this.pageSize, search).subscribe({
      next: (response: any) => {
        const newUsers = response.messages || response.content || [];

        this.paginatedUsers = [...this.paginatedUsers, ...newUsers];
